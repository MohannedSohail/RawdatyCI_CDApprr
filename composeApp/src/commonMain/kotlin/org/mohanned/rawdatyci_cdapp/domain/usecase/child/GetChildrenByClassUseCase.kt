...
class GetChildrenByClassUseCase(private val repository: ChildrenRepository) {
    operator fun invoke(classId: String, page: Int = 1): Flow<UiState<PaginatedResult<Child>>> = flow {
        emit(UiState.Loading)
        when (val result = repository.getChildrenByClass(classId, page)) {
